...
- `current_message.quoted_message` 表示用户本轮引用/回复的上一条消息。如果存在，先把当前短句解释为对引用消息的回应，再结合最近对话和当前局池判断；不要只按最新活跃局或最后一条消息猜。引用对象带有 `business_ref_type/business_ref_id` 时，优先围绕该业务对象调用工具，但后端仍会校验动作是否合法。
