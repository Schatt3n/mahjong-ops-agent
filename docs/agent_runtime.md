...
旧 trial/workflow 代码和历史 v2 链路仅作为 legacy/reference 保留，不接入当前主服务、上下文构建、工具网关或默认评测入口。当前稳定 import 面、主实现包和默认启动入口都收敛为 `mahjong_agent_runtime`。`mahjong_agent_v3` 只作为兼容壳保留，不能作为主链路继续开发。
