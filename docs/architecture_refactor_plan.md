...
- `ControlledWorkflowService` 的 `QUEUE_INVITES` 状态迁移已改为读取 `CREATE_GAME` 工具返回的 `state_write_intent.target_status/requirement/entity_id`；主流程不再根据 outbox 是否创建成功自行重算 `open -> negotiating`。工具层提出状态写入意图，状态机校验合法性，状态账本负责落库。
