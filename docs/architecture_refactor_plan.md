...
- `trial_followup.py` 已新增 `TrialOrganizerFollowupAdapter`，把候选人协商后给发起人的 followup 编排从脚本移出：它只负责 `send_message/create_pending_followup` 的后端工具校验、待审批 followup 创建、tool audit 和 action plan 投影；不会直接外发，也不改变局状态。当前 LLM 协商草稿 prompt、followup 文案 guard 和 SQLite 写入函数仍由脚本回调提供，后续可继续迁移到 `reply_policy`/发送网关。
