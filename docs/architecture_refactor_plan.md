...
- `ControlledWorkflowService` 的 `QUEUE_INVITES/CLOSE_GAME` 状态迁移已改为读取工具返回的 `state_write_intent.target_status/requirement/entity_id`；主流程不再根据 outbox 是否创建成功自行重算 `open -> negotiating`，也不再把关闭局硬编码为 `cancelled`。工具层提出状态写入意图，状态机校验合法性，状态账本负责落库。
