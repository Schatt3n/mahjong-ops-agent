...
- `trial_tool_planning.py` 已新增 `TrialToolPlanPromptBuilder`、`TrialToolPlanPromptInput` 和 `TrialToolCallNormalizer`，把 Web 试用台旧 LLM 工具规划 prompt/payload contract 与 tool_calls 归一化从脚本移出；它们只构造发给模型的结构化上下文、过滤未知/重复工具、强制 `send_message` 进入待审批模式，不调用模型、不执行工具、不做预算，后续应并入统一 `ToolOrchestrator` 工具选择 contract。
