...
- `trial_tool_planning.py` 已新增 `TrialToolPlanPromptBuilder`、`TrialToolPlanPromptInput`、`TrialToolCallNormalizer` 和 `TrialToolActionProposalFactory`，把 Web 试用台旧 LLM 工具规划 prompt/payload contract、tool_calls 归一化和受控 action proposal 生成从脚本移出；它们只构造发给模型的结构化上下文、过滤未知/重复工具、强制 `send_message` 进入待审批模式、生成稳定 `action_id/idempotency_key`，不调用模型、不执行工具、不做预算、不决定允许/拒绝，后续应并入统一 `ToolOrchestrator` 工具选择 contract。
