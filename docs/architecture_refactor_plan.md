...
- `SemanticResolver` 已开始执行语义 contract 验收：`intent/proposed_action/confidence/reasoning_summary/slots` 是必需字段，`slots/action_arguments/profile_observations` 必须是约定类型，`intent/proposed_action` 必须在白名单内；每个 slot 也必须是 `SlotValue` contract，包含 `value/source/confidence/confirmed/needs_confirmation`，且 source、confidence、布尔字段类型都要合法。模型缺少 `proposed_action` 或输出半结构化 slot 时不再由后端根据 intent/默认值猜动作和槽位，而是记录 `llm_contract.accepted=false`、写入 `contract_errors` 并转人工，防止不完整模型输出继续进入状态机和工具链。
