...
- `trial_tool_planning.py` 已新增 `TrialToolPlanPromptBuilder` 和 `TrialToolPlanPromptInput`，把 Web 试用台旧 LLM 工具规划 prompt/payload contract 从脚本移出；它只构造发给模型的结构化上下文，不调用模型、不执行工具、不做预算，后续应并入统一 `ToolOrchestrator` 工具选择 contract。
