...
- `trial_reply.py` 已新增 `TrialReplyDraftAdapter`，把 Web 试用台旧链路中“工具和状态上下文就绪后生成建议回复，并把回复写回短期记忆”的回复阶段从 `analyze()` 移出；当前内部仍回调旧 `_suggested_reply`、LLM prompt、预算和 guard，后续应继续拆到统一 `ReplyPolicy`/`ReplyGuard`。
